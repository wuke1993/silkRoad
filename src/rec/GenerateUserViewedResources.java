 ...
 * ]
 */
public class GenerateUserViewedResources {
	public static void main(String[] args) {
		// first time using for batch processing
		long start = System.currentTimeMillis();
		
		List<Document> logsDocuments = 
				GenerateUserViewedResources.readLogsFromMongodb("logs", "newsLogs");
		Map<String, TreeSet<String>> userResourcesMap = 
				GenerateUserViewedResources.processLogsDocuments(logsDocuments);
		Map<String, ArrayList<String>> historyUserResourcesMap = 
				GenerateUserViewedResources.readHistoryUserResourcesFromMongodb("", null);
		Map<String, ArrayList<String>> newUserCoursesMap = 
				GenerateUserViewedResources.mergeUserCourses(historyUserCoursesMap, userCoursesMap);
		GenerateUserViewedResources.storeUserCoursesIntoMongodb(newUserCoursesMap);
	...
	static List<Document> readLogsFromMongodb(String dbName, String collectionName) {
		...
		MongoCollection<Document> collection = MongodbConn.getMongoCollection(dbName, collectionName);
	...
	static List<Document> readOneDayLogsFromMongodb(String dbName, String collectionName, String date) {
		...
		Pattern pattern = Pattern.compile("^" + date + ".*$"); // ×óÆ¥Åä
	...
	static void storeUserResourcesIntoMongodb(String dbName, String collectionName, Map<String, ArrayList<String>> userResourcesMap) {
		...
		collection = MongodbConn.getMongoCollection(dbName, collectionName);
